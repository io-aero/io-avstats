...
def check_arg_msaccess(args: argparse.Namespace) -> None:
    ...
    if ARG_TASK in [
        ...
        if not args.msaccess:
            ...
            terminate_fatal(glob.ERROR_00_901.replace("{task}", ARG_TASK))
    else:
        if args.msaccess:
            # ERROR.00.902 The task '{task}' does not require an
            # argument '-m' or '--msaccess'
            terminate_fatal(glob.ERROR_00_902.replace("{task}", ARG_TASK))
        else:
            return

    ARG_MSACCESS = args.msaccess.lower()

    if ARG_MSACCESS == glob.ARG_MSACCESS_AVALL:
        return

    if ARG_MSACCESS == glob.ARG_MSACCESS_PRE2008.lower():
        ARG_MSACCESS = glob.ARG_MSACCESS_PRE2008
        return

    if "." in ARG_MSACCESS:
        # ERROR.00.904 '{msaccess}': the MS Access database file name must not
        # contain a file extension
        terminate_fatal(glob.ERROR_00_904.replace("{msaccess}", args.msaccess))
    ...
    if not (
        ...
        terminate_fatal(glob.ERROR_00_903.replace("{msaccess}", args.msaccess))
...
def check_arg_msexcel(args: argparse.Namespace) -> None:
    ...
    if ARG_TASK in [glob.ARG_TASK_L_C_D]:
        if not args.msexcel:
            # ERROR.00.923 The task '{task}' requires valid
            # argument '-e' or '--msexcel'
            terminate_fatal(glob.ERROR_00_923.replace("{task}", ARG_TASK))
    else:
        if args.msexcel:
            # ERROR.00.924 The task '{task}' does not require an
            # argument '-e' or '--msexcel'
            terminate_fatal(glob.ERROR_00_924.replace("{task}", ARG_TASK))
        else:
            return

    directory_name = ""
    if ARG_TASK == glob.ARG_TASK_L_C_D:
        directory_name = io_config.settings.correction_work_dir

    ARG_MSEXCEL = args.msexcel

    if ARG_TASK in [glob.ARG_TASK_L_C_D, glob.ARG_TASK_L_C_D]:
    ...
    try:
        ...
        terminate_fatal(glob.ERROR_00_932.replace("{filename}", file_name))
    except (InvalidFileException, zipfile.BadZipFile):
        # ERROR.00.925 '{msexcel}' is not a valid Microsoft Excel file
        terminate_fatal(glob.ERROR_00_925.replace("{msexcel}", file_name))
...
def cleansing_postgres_data() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_065)
...
def correct_dec_lat_lng() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_040)
    io_utils.progress_msg("-" * 80)
...
def create_db_schema() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_044)
    io_utils.progress_msg("-" * 80)
...
def download_ntsb_msaccess_file(msaccess: str) -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_047.replace("{msaccess}", msaccess))
    io_utils.progress_msg("-" * 80)
...
def find_nearest_airports() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_086)
    io_utils.progress_msg("-" * 80)
...
def get_args() -> None:
    ...
    if parsed_args.msexcel:
        ...
        progress_msg(
            glob.INFO_00_041.replace("{task}", glob.ARG_TASK)
            .replace("{value_task}", parsed_args.task)
            .replace("{msexcel}", glob.ARG_MSEXCEL)
        ...
        progress_msg(
            glob.INFO_00_008.replace("{task}", glob.ARG_TASK)
            .replace("{value_task}", parsed_args.task)
            .replace("{msaccess}", glob.ARG_MSACCESS)
...
def load_aviation_occurrence_categories() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_073)
    io_utils.progress_msg("-" * 80)
...
def load_airport_data() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_085)
    io_utils.progress_msg("-" * 80)
...
def load_correction_data(filename: str) -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_042.replace("{filename}", filename))
    io_utils.progress_msg("-" * 80)
...
def load_country_state_data() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_057)
    io_utils.progress_msg("-" * 80)
...
def load_ntsb_msaccess_data(msaccess: str) -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_049.replace("{msaccess}", msaccess))
    io_utils.progress_msg("-" * 80)
...
def load_sequence_of_events() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_075)
    io_utils.progress_msg("-" * 80)
...
def load_simplemaps_data() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_050)
    io_utils.progress_msg("-" * 80)
...
def load_zip_code_db_data() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_056)
    io_utils.progress_msg("-" * 80)
...
def refresh_db_schema() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_071)
    io_utils.progress_msg("-" * 80)
...
def update_db_schema() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_045)
    io_utils.progress_msg("-" * 80)
...
def verify_ntsb_data() -> None:
    ...
    io_utils.progress_msg(glob.INFO_00_043)
    io_utils.progress_msg("-" * 80)
...
def version() -> str:
    ...
    return glob.IO_AERO_DB_VERSION
