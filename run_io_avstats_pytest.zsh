...
if [[ "${IO_AERO_TASK}" = "l_n_a" ]]; then
    if [[ -z "$2" ]]; then
        ...
        echo "---------------------------------------------------------"
        read "IO_AERO_MSACCESS?Enter the stem name of the desired MS Access database file "
...
case "${IO_AERO_TASK}" in
    "a_o_c"|"c_d_s"|"c_l_l"|"c_p_d"|"f_n_a"|"l_a_p"|"l_c_s"|"l_s_d"|"l_s_e"|"l_z_d"|"r_d_s"|"u_d_s"|"v_n_d"|"version")
...
# ------------------------------------------------------------------------------
    "c_d_l")
        if ! ( ./scripts/run_docker_compose_local.zsh ${IO_AERO_COMPOSE_TASK} ); then
            exit 255
        fi
        ;;
# ------------------------------------------------------------------------------
