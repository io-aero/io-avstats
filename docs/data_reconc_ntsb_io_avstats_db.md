# NTSB versus IO-AVSTATS-DB

## 1. General

The reconciliation of the databases of **IO-Aero** and **NTSB** can always be done only at the first of the month, because the **NTSB** updates the database **avall** only at this time.
The MS Excel file **data\Database Matching\database_matching.xlsx** is used for reconciliation.

## 2. IO-AVSTATS-DB

The reconciliation data from the **IO-Aero** Database **IO-AVSTATS-DB** is determined with the following database query:
...
To do this, these databases must first be downloaded from the [NTSB website](https://data.ntsb.gov/avdata){:target="_blank"} with task **d_n_a**. 

The reconciliation data of both **NTSB** databases **Pre2008** and **avall** are determined with the following database query:
...
## 4. Latest reconciliation (February 1, 2023) 

<kbd>![](img/NTSB_vs_IO_AVSTATS_23_02.jpg)</kbd>
