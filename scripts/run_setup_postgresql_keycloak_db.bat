...
rem ------------------------------------------------------------------------------
rem
rem run_setup_postgresql_keycloak_db.bat: Set up the keycloak_db PostgreSQL
...
echo POSTGRES_DBNAME          : %IO_AERO_POSTGRES_KEYCLOAK_DBNAME_ADMIN%
...
echo PostgreSQL
echo --------------------------------------------------------------------------------
echo Docker create %IO_AERO_POSTGRES_KEYCLOAK_CONTAINER_NAME% (PostgreSQL %IO_AERO_POSTGRES_VERSION%)

if not exist %IO_AERO_POSTGRES_KEYCLOAK_PGDATA%\ (
    mkdir %IO_AERO_POSTGRES_KEYCLOAK_PGDATA%
)
...
docker create -e        POSTGRES_DB=%IO_AERO_POSTGRES_KEYCLOAK_DBNAME_ADMIN% ^
              ...
              -v        "%cd%\%IO_AERO_POSTGRES_KEYCLOAK_PGDATA%":/var/lib/postgresql/data ^
              --restart always ^
              postgres:%IO_AERO_POSTGRES_VERSION%

echo Docker start %IO_AERO_POSTGRES_KEYCLOAK_CONTAINER_NAME% (PostgreSQL %O-AVSTATS_POSTGRES_VERSION%) ...
docker start %IO_AERO_POSTGRES_KEYCLOAK_CONTAINER_NAME%
